...
/*Task 1: counterMaker()
  ...
  The difference between counter1 and counter2 is the location of the variable 'count'. count exists only within the function for counter1, but exists in the global scope for counter2.

  2. Which of the two uses a closure? How can you tell?
  The code for counter1 uses a closure because there is a function within a function. 
  
  3. In what scenario would the counter1 code be preferable? In what scenario would 
     counter2 be better?  
     counter1 allows for new variables to be created to use the function, so keeping track of scores for multiple players in a video game would be possible, for example. For counter2, the count variable exists in the global scope so it would be better used in a case where you want the 'count' variable available for use in other places in your code. 
...
function finalScore(callback, innings){
  ...
  for (let i = 0; i < innings; i ++) {
    totalScore.Home = totalScore.Home + callback();
    totalScore.Away = totalScore.Away + callback();
  }
  return totalScore;
}
finalScore(inning, 9);
...
function scoreboard(getInningsCallback, inningCallback, inningsNumber) {
  const scoreArray = [];
  let totalScore = 0
  let homeTotal = 0
  let awayTotal = 0

  for (let i = 1; i < inningsNumber + 1; i++) {
    let homeScore = inningCallback();
    homeTotal += homeScore;
    let awayScore = inningCallback();
    awayTotal += awayScore;
    scoreArray[i] = `Inning ${i}: Away ${awayScore} - Home ${homeScore}`;
  }
  if (awayTotal != homeTotal) {
    scoreArray[inningsNumber + 1] = `Final Score: Away ${awayTotal} - Home ${homeTotal}`;
  }
